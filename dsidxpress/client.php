<?php
add_action("wp", array("dsSearchAgent_Client", "CheckIsIDXHome"));
...
class dsSearchAgent_Client {
	...
	static function CheckIsIDXHome(){
		global $post;
		if( is_front_page() && is_singular( 'ds-idx-listings-page' ) ){
			self::$IsIDXHomePage = true;
			remove_action('wp_head', 'rel_canonical');
			add_action('wp_head', 'dsSearchAgent_Client::FixCanonicalForHomePage');
		}
	}
	...
	static function Activate($posts, $params=array(), $idx_page_id=false) {
		...
		// Removing short link from the plugin
	...
	static function FrameAction($action, $get, $idx_page_id=false){
		...
		if (isset($options["AdvancedTemplate"]))
			wp_cache_set($post_id, array("_wp_page_template" => array($options["AdvancedTemplate"])), "post_meta");

		$description = NULL;
		$title = NULL;

		wp_enqueue_script('dsidxpress_frame_action', 'https://idx.diversesolutions.com/scripts/controls/Remote-Frame.aspx?MasterAccountID='. $options['AccountID'] .'&amp;SearchSetupID='. $options['SearchSetupID'] .'&amp;LinkID=0&amp;Height=2000', array(), null, true);		
		...
		$post = (object)array(
			...
			"ping_status"		=> "closed",
			"post_author"		=> 1,
			"post_content"		=> '<div id="_dsSearchAgent_Idx_Frame_0_div"></div>',
			"post_date"			=> date("c"),
	...
	static function ApiAction($action, $get, $idx_page_id=null) {
		global $wp_query;	
		global $dsIdxOptions;
		$options = $dsIdxOptions;
		
		$post_id = !$idx_page_id?time():$idx_page_id;

		add_action("wp_head", array("dsSearchAgent_Client", "Header"), 9);
		...
		$account_options = dsSearchAgent_GlobalData::GetAccountOptions(true);
				
		if ($action == "results" || $action == "search") dsidxpress_autocomplete::AddScripts(false);

		if ($action == "results") {
			// save search
			if(isset($get["idx-save"]) && !empty($get["idx-save"]) && $get["idx-save"] == "true") {
				$apiParams["name"] = $get["idx-save-name"];
				$apiParams["updates"] = isset($get["idx-save-updates"]) && $get["idx-save-updates"] == "on" ? "true" : "false";
			...
			if (isset($apiParams["query.PropertySearchID"]))
				$apiParams["query.ForceUsePropertySearchConstraints"] = "true";
			$apiParams["directive.ResultsPerPage"] = 30;
			...
			$apiParams["responseDirective.IsResultsPageModernView"] = $account_options->IsResultsPageModernView;
			$apiParams["responseDirective.ShowMapInResultsPage"] = $account_options->ShowMapInResultsPage;
			$apiParams["responseDirective.MapOrientationInResultsPage"] = $account_options->MapOrientationInResultsPage;
			...
			if(isset($_COOKIE['dsidx-visitor-auth'])) {
				...
				if (defined("DS_REQUEST_MULTI_AVAILABLE") && DS_REQUEST_MULTI_AVAILABLE==true)  {
					$apiVisitResponse = dsSearchAgent_ApiRequest::FetchData("RecordVisit", $visitParams,false, 0,null,false,true);
					if (empty($apiHttpResponse['body'])) {
						$apiVisitResponse['body'] = '%%ds_RecordVisit|' . json_encode($apiVisitResponse) . '|ds_end%%';
						echo $apiVisitResponse['body'];
					}
				}
				else {
    	...
    	$firstimageWidth = self::ExtractValueFromApiData($apiData, "firstimageWidth");
   		$firstimageHeight = self::ExtractValueFromApiData($apiData, "firstimageHeight");
		self::$meta_tag_data = array('firstimage' => $firstimage,'firstimageWidth'=> $firstimageWidth,'firstimageHeight'=> $firstimageHeight, 'title' => $title, 'description' => $description);
		...
		if (!$idx_page_id && isset($seo_title)) {
			...
			{
				add_filter('pre_get_document_title', array($dsidxpress_seo, 'dsidxpress_title_filter'));
			}
			else
			{
		...
		if (isset($seo_keywords) || isset($seo_description)) {
			add_action('wp_head', array($dsidxpress_seo, 'dsidxpress_head_action'), 9);
		}
	...
	static function EnsureBaseUri() {
		...
		if ($requestedPath != $expectedPath) {
			...
			$sortColumn = (isset($_GET[$sortColumnKey])) ?sanitize_text_field($_GET[$sortColumnKey]) : null;
			$sortDirection = (isset($_GET[$sortDirectionKey])) ? sanitize_text_field($_GET[$sortDirectionKey]) : null;
    ...
    if (strpos($homeURL, 'https://') !== false)
      $blogUrlWithoutProtocol = str_replace("https://", "", $homeURL);
    else 
		  $blogUrlWithoutProtocol = str_replace("http://", "", $homeURL);
	...
	static function Header() {
		...
		if (self::$CanonicalUri && !$thesis && !self::$IsIDXHomePage)
			remove_action('wp_head', 'rel_canonical');
			echo "<link rel=\"canonical\" href=\"" . esc_url(self::GetPermalink()) . "\" />\n";
	}
	static function FixCanonicalForHomePage(){
		$link = get_home_url();
	...
	static function SocialMetaTags() {
		$firstimage = esc_attr(self::$meta_tag_data['firstimage']);
		$firstimageWidth = esc_attr(self::$meta_tag_data['firstimageWidth']);
		$firstimageHeight = esc_attr(self::$meta_tag_data['firstimageHeight']);
		$title = esc_attr(self::$meta_tag_data['title']);
		$description = esc_attr(self::$meta_tag_data['description']);
		if (!empty($firstimage)) {
			echo "<meta property='og:image' content='" . $firstimage . "' />";
			echo "<meta property='og:image:type' content='image/jpeg' />";
			if (!empty($title))
				echo "<meta property='og:image:alt'  content='" . $title . "'  />";
			if (!empty($firstimageWidth))
				echo "<meta property='og:image:width' content='" . $firstimageWidth . "' />";
			if (!empty($firstimageHeight))
				echo "<meta property='og:image:height' content='" . $firstimageHeight . "' />";
		}
		$blogUrl = get_home_url();
		//Twitter Card
		echo <<<HTML
