...
dsIDXpressOptions = {
	...
	FilterViews: function () {
		...
		var showConsentOriginal = jQuery('#dsidxpress-RegistrationShowConsent-Original');
		var showConsentModified = jQuery('#dsidxpress-RegistrationShowConsent');
		if(showConsentOriginal && showConsentModified)
		{
			if(showConsentOriginal.val().toLowerCase() != showConsentModified.val().toLowerCase())
			{
				var showConsentUpdatedDate = jQuery('#dsidxpress-RegistrationConsentLastUpdatedDate');
				if(showConsentUpdatedDate)
				{
					var utcDateNow = new Date().toISOString().replace(/T/, ' ').replace(/Z/, '');
					showConsentUpdatedDate.val(utcDateNow);
				}
			}
		}
	},
	...
	Init: function () {
		...
		jQuery('.dsidxpress-api-radio').click(function () {
			jQuery('#' + this.id.substring(0, this.id.lastIndexOf("-"))).val(this.value);
		});		
