(function ($) {
    dsIdxLinkBuilder = {
        init: function () {
            ...
            if (dsIDXPressPackage && dsIDXPressPackage.toLowerCase() == 'pro')
                isProAccount = true;
            if ($('#dsidxpress-assembled-url').val() == 'http://' || $('#dsidxpress-assembled-url').val() == 'https://') {
                this.resetUrl();
            } else {
        ...
        buildEditor: function (el, n) {
            ...
            if ($e.val() != '-1') {
                ...
                $.each(filter.fields, function (i, field) {
                    ...
                    switch (field.mode) {
                        ...
                        case 'lookupPropTypes':
                            ...
                            if (lb.current_filters[field.id]) {
                                var txt = new String(lb.current_filters[field.id]);
                                curSelections = txt.split(",");
                            }
                            var ii;
                            ...
                            html += '</div>';
                            break;
                            case 'lookupPropFeatures':                                
                                var curSelections = [];
                                value = (field.id in lb.current_filters) ? lb.escapeHtmlEntities(lb.current_filters[field.id].join('\n')) : '';
                                html += '<div id="' + fid + '" name="' + fid + '" class="regular-text">';
                                var curValues = document.getElementById('linkBuilderPropertyFeatures').value;
                                var arrayValues = curValues.split(",");
                                if (lb.current_filters[field.id]) {
                                    var txt = new String(lb.current_filters[field.id]);
                                    curSelections = txt.split(",");
                                ...
                                for (i = 0; i < arrayValues.length; i++) {
                                    ...
                                    html += '<input type="checkbox" name="groupPropFeature" value="' + indyValues[0] + '"' + select + '>' + indyValues[1] + '<div style="height:6px; clear:both;"></div>';
                                }
                                html += '</div>';
                                break;
                        case 'lookupListStatuses':
							...
							if(dsIDXPressPackage && dsIDXPressPackage.toLowerCase() == 'pro'){
								...
								if(('HasConditionalData' in mlsCapabilities) && mlsCapabilities['HasConditionalData'] !== ''){
									html += '<input type="checkbox" name="groupListStatus" value="2"' + selectConditional + '>Conditional<div style="height:6px; clear:both;"></div>';
								}
								if(('HasPendingData' in mlsCapabilities) && mlsCapabilities['HasPendingData'] !== ''){
                        ...
                        case 'customPolygon':
							...
							{
								value = (field.id in lb.current_filters) ? lb.escapeHtmlEntities(lb.current_filters[field.id]) : '';
								html += '<div id="dsMap-canvas" style="width: auto;height: 500px;"></div>';
								html += '<input id="' + fid + '" name="' + fid + '" type="hidden" title="' + hint + '" value="' + value + '" readonly style="width:100%"/>';
								bPolygon = true;
								sPolygon = value;
							}
                            break;
                ...
                if (bPolygon === true) {
                    dsCustomPolygonInitialize(sPolygon)
                }
        ...
        addFilter: function () {
            ...
            $.each(filter.fields, function (x, field) {
                ...
                else if (field.type == 'lookupPropFeatures') {//this type has different way of aquiring the value from radio inputs
                    if (lb.validate(field, $f.val())) {
                        var curChildren = $f.children('input');
                        lb.current_filters[field.id] = [];
                        var index = 0;
        ...
        filters: {
                ...
                fields: [
                    {
                        ...
                        type: 'lookupPropFeatures',
                        mode: 'lookupPropFeatures',
            ...
            cities: {
                ...
                autocomplete: {
                    label: 'Search Cities',
                    url: dsidxAjaxHandler.ajaxurl + '?action=dsidx_client_assist&dsidx_action=LoadAreasByType',
            ...
            custom_polygon: {
                name: 'Custom Polygon',
                help: 'If you want to delete the polygon and start over, just update the filter, and then remove it and add it again.',
                fields: [
                    {
                        label: 'Custom encoded Polygon',
                        id: 'idx-q-CustomPolygons',
                        type: 'string',
                        mode: 'customPolygon',
                        hint: 'Use the map to draw your area'
					}
				]
            },
            sort_columns: {
    ...
    var dsCustomPolygon = null;
    var dsMapZoom = 8;
    var initialPolygon=null; 
    function initializeMap()
    {
        // Map Center
        // this needs to be updated according MLS 
        let dsIDXMapLatitude = '47.643186105437';
        let dsIDXMapLongitude = '-122.305297851563';
        if(dsIDXSeachSetupMapData) {
            dsIDXMapLatitude= dsIDXSeachSetupMapData[0].MapStartLatitude;
            dsIDXMapLongitude= dsIDXSeachSetupMapData[0].MapStartLongitude;
        }
        var myLatLng = new google.maps.LatLng(dsIDXMapLatitude, dsIDXMapLongitude);
        // General Options
        var mapOptions = {
            zoom: dsMapZoom, //Looking at the whole US
            center: myLatLng,
        ...
        dsCustomPolygon = new google.maps.Map(document.getElementById('dsMap-canvas'), mapOptions);

    }

    function initializeDrawingManger()
    {
        var polyOptions = {
            strokeWeight: 1,
            fillOpacity: 0.35,
    ...
    function setUpDrawingManager(mode,map)
    {
    ...
    function setupListeners(initialPoly)
    {
        google.maps.event.addListener(drawingManager, 'overlaycomplete', function (e) {
            ...
            newShape.type = e.type; //polygon
            getDsShapeCoords(newShape);
            if (initialPoly && initialPoly.length > 0) {
                var bounds = new google.maps.LatLngBounds();
                initialPolygon.getPath().forEach(function (LatLng) {
                    bounds.extend(LatLng);
                });
                dsCustomPolygon.fitBounds(bounds);
                dsCustomPolygon.panToBounds(bounds);
            }
            ...
            if (newShape !== null && newShape.type === "polygon") {
                var path = newShape.getPath();
                google.maps.event.addListener(path, "insert_at", function (e) {
                    getDsPolygonCoords(newShape);
                });
        ...
        google.maps.event.addListener(dsCustomPolygon, 'bounds_changed', function (e) {
            dsMapZoom = dsCustomPolygon.getZoom();
        });
    }
    function dsCustomPolygonInitialize(initialPoly) {
        
        initializeMap();
        if(dsCustomPolygon) {
            initializeDrawingManger(google.maps.drawing.OverlayType.POLYGON,dsCustomPolygon);
            var drawingControlDiv = document.createElement('div');
            var drawingControl = new DrawingControl(drawingControlDiv, dsCustomPolygon);
            drawingControlDiv.index = 1;
            dsCustomPolygon.controls[google.maps.ControlPosition.RIGHT_BOTTOM].push(drawingControlDiv);
            if (initialPoly.length > 0) {
                var initialPath = new google.maps.geometry.encoding.decodePath(initialPoly);
                ...
                initialPolygon.setMap(dsCustomPolygon);
                drawingManager.setDrawingMode(null);
                ...
                var path = initialPolygon.getPath();
                google.maps.event.addListener(path, "insert_at", function (e) {
                    getDsPolygonCoords(initialPolygon);
                });
                google.maps.event.addListener(path, "set_at", function (e) {
                    getDsPolygonCoords(initialPolygon);
                });
                google.maps.event.addListener(path, 'click', function (e) {
                    getDsShapeCoords(initialPolygon);
                });

                dsCustomPolygon.fitBounds(bounds);
                dsCustomPolygon.panToBounds(bounds);
                dsCustomPolygon.setZoom(dsMapZoom);
            }
            setupListeners(initialPoly);
    ...
    function getDsPolygonCoords(shape) {
        dsClearSelection();
        ...
        if (encodeString) {
            var dsTextArea = document.getElementById('idx-q-CustomPolygons');
            dsTextArea.value = encodeString;
        }
        dsMapZoom = dsCustomPolygon.getZoom();
    }

    function getDsShapeCoords(shape) {
        dsClearSelection();
        shape.setEditable(true);
        selectedShape = shape;
        ...
        if (typeof selectedShape.getPath == 'function') {
            ...
            for (var i = 0; i < selectedShape.getPath().getLength(); i++) {
                pathstr = "";
                pathstr = selectedShape.getPath().getAt(i).toUrlValue(8);
                pathstr = pathstr.split(',');
                sectionObj.lat = parseFloat(pathstr[0]);
                sectionObj.lng = parseFloat(pathstr[1]);
                pathArray.push(new google.maps.LatLng(sectionObj.lat, sectionObj.lng));
                sectionObj = {};
            }
        }

        var encodeString = google.maps.geometry.encoding.encodePath(pathArray);
        if (encodeString) {
            var dsTextArea = document.getElementById('idx-q-CustomPolygons');
            dsTextArea.value = encodeString;
        }
    ...
    /**
       * The DrawingControl adds a control to the map to enable editing in map* 
    ...
    controlUI.title = 'Click to draw a custom region';
    ...
    controlText.innerHTML = '<img src=https://assets.diversesolutions.com/Images/Icons/pencil.png>';
    ...
    controlUI.addEventListener('click', function() {
        ...
        setUpDrawingManager(google.maps.drawing.OverlayType.POLYGON,dsCustomPolygon);
    });
    }
