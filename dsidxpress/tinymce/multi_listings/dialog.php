...
$propertyFeatures = dsSearchAgent_ApiRequest::FetchData("PropertyFeatures", array(), false, 60 * 60 * 24);
$propertyFeatures = json_decode($propertyFeatures["body"]);
...
<head>
	...
	<?php
		...
		wp_enqueue_script('dsidxpress_tiny_mce_mctabs', $localJsUri . 'tinymce/utils/mctabs.js', array(), $tinymce_version);
		wp_enqueue_script('jquery');
		wp_enqueue_script('dsidxpress_multi_listings', DSIDXPRESS_PLUGIN_URL . 'tinymce/multi_listings/js/dialog.js', array('jquery'), DSIDXPRESS_PLUGIN_VERSION);

		wp_add_inline_script('dsidxpress_multi_listings', '
			var ApiRequest = {
				uriBase: "' .  admin_url( 'admin-ajax.php' ) . '?action=dsidx_client_assist'  . '",
				searchSetupID: ' . esc_html($options["SearchSetupID"]) . '
			};
		...
		<div id="custom_search_panel" class="panel current">
			<table style="width: 100%;">
				...
				<tr>
					<th>MLS #s</th>
					<td>
						<input type="text" id="mls-numbers" style="width: 100%;box-sizing: border-box;" />
					</td>
				</tr>
				<tr>
					<th id ="result-label" />
				...
				<tr>
					<th>Address Masks</th>
					<td>
						<input type="text" id="address-masks" placeholder="e.g. %Mountain Dr%, %Main St%" style="width: 100%;box-sizing: border-box;" />
					</td>
				</tr>
				<tr>
					<th>Price range</th>
...
?>
				...
				<tr>
					...
					<td>
						<div id="property-feature-container">
<?php
if (!empty($propertyFeatures)) {
	foreach ($propertyFeatures as $propertyFeature) {
		$name = htmlentities($propertyFeature->DisplayName);
		$id = $propertyFeature->SearchSetupFeatureID;
		echo <<<HTML
							<input type="checkbox" name="property-feature-{$id}" id="property-feature-{$id}" value="{$id}" />
							<label for="property-feature-{$id}">{$name}</label>
							<br />
HTML;
	}
}
...
if (!empty($availableLinks)) {
	foreach ($availableLinks as $link) {
		echo "<option value='" . esc_attr($link->LinkID) . "' >" . esc_html($link->Title) . "</option>";
	}
}
?>
				</select>
