...
HTML;
		...
		if (is_array($propertyTypes)) {
			foreach ($propertyTypes as $propertyType) {
				...
				$searchSetupPropertyTypeID= esc_attr($propertyType->SearchSetupPropertyTypeID);
				echo "<option value=\"{$searchSetupPropertyTypeID}\">{$name}</option>";
			}
		}
...
HTML;
			foreach ($searchOptions['communities'] as $community) {
				...
				echo "<option value=\"" . esc_attr($community) . "\">" . esc_html($community) . "</option>";
			}
			echo '</select>';
		}
...
HTML;
			foreach ($searchOptions["tracts"] as $tract) {
				// there's an extra trim here in case the data was corrupted before the trim was added in the update code below
				$tract = trim($tract);
				echo "<option value=\"" . esc_attr($tract) . "\">" . esc_html($tract) . "</option>";
			}
			echo '</select>';
		}
...
HTML;
			foreach ($searchOptions["zips"] as $zip) {
				// there's an extra trim here in case the data was corrupted before the trim was added in the update code below
				$zip = trim($zip);
				echo "<option value=\"" . esc_attr($zip) . "\">" . esc_html($zip) . "</option>";
			}
			echo '</select>';
		}
...
HTML;
		...
		if($account_options->EulaLink){
			$eula_url = esc_url($account_options->EulaLink);
			echo <<<HTML
