...
add_action("init", array("dsSearchAgent_Rewrite", "TrailingSlashIt"));
...
class dsSearchAgent_Rewrite {
	...
	static function TrailingSlashIt(){
		/*
		This is a fix for search results that are missing the trailing slash on the /idx url.
		...
		*/
		...
		if( substr( $uri, -4 ) == '/idx' ){
			...
			$url = $protocol . $host . $uri . '/' . $querystring;
			wp_redirect( $url, 301 );
			exit();
		}
	}
