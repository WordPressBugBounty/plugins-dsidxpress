...
if (is_admin()) {
        ...
        add_filter( 'get_pages', array( 'dsIdxListingsPages', 'add_ds_idx_listings_page_to_dropdown'), 10, 2);
} else {
...
add_action( 'pre_get_posts', array( 'dsIdxListingsPages', 'enable_front_page_ds_idx_listings_page'));

require_once(DSIDXPRESS_PLUGIN_PATH.'dsLinkUpdater.php'); /*DIV-15200 : This class will update all diverse solution http links to https*/
...
class dsIdxListingsPages {
    ...
    public static function DisplayPage($posts) {
            global $wp_query;
            if (is_array($wp_query->query) && isset($posts[0]) && $posts[0]->post_type == 'ds-idx-listings-page') {
                ...
                $filters = array_combine(
                    ...
                    array_map(
                        function($key, $value) {
                            if(isset($key) && strtolower($key) == strtolower("idx-q-CustomPolygons"))
                            {
                                return $value;
                            }
                ...
                if(strlen(trim($pageContent . $newPosts[0]->post_content))>0)
                {
                    if(substr( get_home_url(), 0, 8 ) === "https://")
                        $newPosts[0]->post_content = dsIdxLinkUpdater::UpdateDSLinksToSSL(trim($pageContent.$newPosts[0]->post_content)); 
                    else 
            ...
            if(isset($posts) && array_key_exists(0, $posts) && strlen(trim($posts[0]->post_content))>0)
            {
                if(substr(get_home_url(), 0, 8 ) === "https://")                
                    $posts[0]->post_content =  dsIdxLinkUpdater::UpdateDSLinksToSSL(trim($posts[0]->post_content));
            }
            return $posts;
    }
    ...
    public static function saveIdxOptions($post_id){
        ...
        if(isset($_POST["ds-idx-page_nonce"])) {
			$ds_idx_page_nonce =  sanitize_text_field($_POST["ds-idx-page_nonce"]);
         }
         
         if(isset($_POST["post_type"])) {
			$post_type =  sanitize_text_field($_POST["post_type"]);
	 	}
        if (empty($ds_idx_page_nonce)) return;

        if (!wp_verify_nonce( $ds_idx_page_nonce, plugin_basename( __FILE__ ) ) ) die('no nonce');

        if ( 'ds-idx-listings-page' == $post_type  ) {
        ...
        if(isset($_POST['dsidxpress-assembled-url'])) {
            $url =esc_url_raw($_POST['dsidxpress-assembled-url']);
...
HTML;
        ...
        <div class="postbox">
            <div class="inside">
                <input type="hidden" id="linkBuilderPropertyTypes" value="' . esc_attr($property_types_html) . '" />';
    ...
    static function add_ds_idx_listings_page_to_dropdown( $pages, $r ) {
        if(isset($r['name']) && 'page_on_front' == $r['name']){
            ...
            function prepend_idx_label($idxPage){
                $idxPage->post_title = 'IDX Page - '.$idxPage->post_title;
                return $idxPage;
            }
            array_walk($items, 'prepend_idx_label');
            foreach($items as $i);
    ...
    static function enable_front_page_ds_idx_listings_page( $query ){
        if((empty($query->query_vars['post_type']) && 0 != $query->query_vars['page_id'])){
            $query->query_vars['post_type'] = array( 'page', 'ds-idx-listings-page');
        }
    }
}
?>
