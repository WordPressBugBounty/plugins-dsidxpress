...
class dsIDXWidgets_MapSearch extends WP_Widget {
    ...
    function __construct() {
        global $pagenow;
        parent::__construct("dsidx-mapsearch", "Map Search", array(
    ...
    function widget($args, $instance) {
        ...
        $mapsAPI = 'https://maps.googleapis.com/maps/api/js?v=3&key='.DSIDXPRESS_GOOGLEMAP_API_KEY;

        wp_enqueue_script('googlemaps3', $mapsAPI, array('jquery'), false, true);
        ...
        $zip = esc_html($instance["zip"]);
        $priceMin = esc_html($instance["priceMin"]);
        $priceMax = esc_html($instance["priceMax"]);
        $priceFloor = esc_html($instance["priceFloor"]);
        $priceCeiling = esc_html($instance["priceCeiling"]);
        $bedsMin = esc_html($instance["bedsMin"]);
        $bathsMin = esc_html($instance["bathsMin"]);
        $sqftMin = esc_html($instance["sqftMin"]);
        $statusType = esc_html($instance["statusType"]);
        $rowCountType = esc_html($instance["rowCountType"]);
        $sortType = esc_html($instance["sortType"]);
        $imagesStub = dsWidgets_Service_Base::$widgets_images_stub;
        $apiStub = dsWidgets_Service_Base::$widgets_api_stub;
        $height = esc_html($instance["height"]);
...
HTML;
    ...
    function update($new_instance, $old_instance) {
        ...
        $new_instance["mapsearchOptions"]["state"] = sanitize_text_field($new_instance["state"]);
        $new_instance["mapsearchOptions"]["city"] =  sanitize_textarea_field($new_instance["city"]);
        $new_instance["mapsearchOptions"]["zip"] =  sanitize_text_field($new_instance["zip"]);

        if(isset($new_instance["mapsearchOptions"]["priceMin"]))
            $new_instance["mapsearchOptions"]["priceMin"] =  sanitize_text_field($new_instance["priceMin"]);

        if(isset($new_instance["mapsearchOptions"]["priceMax"]))
            $new_instance["mapsearchOptions"]["priceMax"] =  sanitize_text_field($new_instance["priceMax"]);


        $new_instance["mapsearchOptions"]["priceFloor"] =  sanitize_text_field($new_instance["priceFloor"]);
        $new_instance["mapsearchOptions"]["priceCeiling"] =  sanitize_text_field($new_instance["priceCeiling"]);
        $new_instance["mapsearchOptions"]["bedsMin"] =  sanitize_text_field($new_instance["bedsMin"]);
        $new_instance["mapsearchOptions"]["bathsMin"] =  sanitize_text_field($new_instance["bathsMin"]);

        if(isset($new_instance["mapsearchOptions"]["sqftMin"]))
            $new_instance["mapsearchOptions"]["sqftMin"] =  sanitize_text_field($new_instance["sqftMin"]);

        $new_instance["mapsearchOptions"]["statusType"] =  sanitize_text_field($new_instance["statusType"]);
        $new_instance["mapsearchOptions"]["rowCountType"] =  sanitize_text_field($new_instance["rowCountType"]);
        $new_instance["mapsearchOptions"]["sortType"] =  sanitize_text_field($new_instance["sortType"]);

        if(isset($new_instance["mapsearchOptions"]["eDomain"]))
            $new_instance["mapsearchOptions"]["eDomain"] =  sanitize_text_field($new_instance["eDomain"]);

        $new_instance["mapsearchOptions"]["height"] =  sanitize_text_field($new_instance["height"]);
        $new_instance["mapsearchOptions"]["width"] =  sanitize_text_field($new_instance["width"]);
    ...
    function form($instance) {
        ...
        $city = esc_textarea($instance["city"]);
        ...
        echo <<<HTML
            ...
            <p>
                <label for="{$widthFieldId}">Width</label> (Leave empty for fluid width)
