zWidgetsAdmin = {
    ...
    dsToggleSlideshowView : function (modernView, horzCount) {
        if (modernView.checked) {
            horzCount.attr('disabled', 'disabled');
            horzCount.val('');
        }
        else {
            horzCount.removeAttr('disabled');
            horzCount.val('4');
        }
    },
    viewTypeChanged: function(viewTypeCtrl, widgetTypeName) {
        viewType = jQuery(viewTypeCtrl).val();
        if(viewType.toLowerCase() == 'simple-search') {
            jQuery('input[name="' + widgetTypeName + '"]').prop('disabled', true);
        }
        else {
            jQuery('input[name="' + widgetTypeName + '"]').prop('disabled', false);
        }
    } 
}